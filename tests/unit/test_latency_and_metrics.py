...
class TestOffsetCommitCounters:
    def test_offset_commit_counter_names_exist_and_are_independent(self):
        ...
        success_before = metrics.KAFKA_OFFSET_COMMIT_TOTAL.labels(
            service=metrics.SERVICE_NAME
        )._value.get()
        failed_before = metrics.KAFKA_OFFSET_COMMIT_FAILED_TOTAL.labels(
            service=metrics.SERVICE_NAME
        )._value.get()
        ...
        assert (
            metrics.KAFKA_OFFSET_COMMIT_TOTAL.labels(service=metrics.SERVICE_NAME)._value.get()
            == success_before + 1
        )
        assert (
            metrics.KAFKA_OFFSET_COMMIT_FAILED_TOTAL.labels(
                service=metrics.SERVICE_NAME
            )._value.get()
            == failed_before
        )
