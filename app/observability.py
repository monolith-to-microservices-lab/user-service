...
HTTP_REQUEST_DURATION = Histogram(
    "http_request_duration_seconds", "HTTP request duration in seconds", ["service", "method", "route"]
)
...
class MetricsMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next: Callable):
        ...
        try:
            ...
            route_path = route.path if route is not None and hasattr(route, "path") else request.url.path
            HTTP_REQUESTS_TOTAL.labels(
                service=SERVICE_NAME, method=request.method, route=route_path, status_code=str(status_code)
            ).inc()
            HTTP_REQUEST_DURATION.labels(service=SERVICE_NAME, method=request.method, route=route_path).observe(
                duration
            )
